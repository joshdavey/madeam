...
class Madeam_Exception extends Exception {
  ...
  public static function catchException(Exception $e, $override = array()) {
    ...
    if (Madeam_Config::get('enable_debug') == true) {
      ...
      echo Madeam::request(Madeam::errorController . '/debug?error=' . urlencode(nl2br($message)) . '&backtrace=' . urlencode($e->getTraceAsString()) . '&snippet=' . urlencode($snippet) . '&line=' . urlencode($line) . '&code=' . urlencode($code) . '&file=' . urlencode($file) . '&documentation=' . 'comingsoong&_layout=1');
      exit();
    } else {
      // return 404 error page
      echo Madeam::request(Madeam::errorController . '/http404?_layout=1');
      exit();
    }
  }
}
